...
def _map_section(kind: str, data: dict[str, Any], source: str, endpoint: str | None) -> dict[str, Any]:
    ...
    if kind == "summary-visualization":
        ...
        section["outliers"] = [{"task": _task_ref(row, endpoint), "duration_seconds": _number(row.get("duration_seconds")) or 0, "preceding": [_task_ref(v, endpoint) for v in row.get("preceding") or []], "following": [_task_ref(v, endpoint) for v in row.get("following") or []], "sequence_signature": _text(row.get("sequence_signature"))} for row in data.get("outliers") or []]

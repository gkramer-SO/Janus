...
function ChartFrame({ title, detail, children }: { title: string; detail: string; children: ComponentChildren }) {
...
export function DotPlot({ title, data, formatValue = text }: { title: string; data: ChartDatum[]; formatValue?: (value: number) => string }) {
  ...
  return <ChartFrame title={title} detail={`${visible.length} highest-signal item${visible.length === 1 ? "" : "s"}`}>
...
export function DonutChart({ title, data }: { title: string; data: ChartSegment[] }) {
  const [active, setActive] = useState(0);
  ...
  return <ChartFrame title={title} detail={`${total.toLocaleString()} result${total === 1 ? "" : "s"}`}>
    <div class="donut-layout">
      ...
      <div class="chart-legend">{arcs.map((datum, index) => <button type="button" class={index === active ? "active" : ""} key={datum.label} onMouseEnter={() => setActive(index)} onFocus={() => setActive(index)} onClick={() => setActive(index)}><i class={datum.tone ?? "accent"} /><span>{datum.label}</span><strong>{datum.percentOfTotal.toFixed(1)}%</strong></button>)}</div>
    </div>
  </ChartFrame>;
}

export function TimelineChart({ title, data }: { title: string; data: ChartDatum[] }) {
  ...
  const barWidth = Math.max(3, Math.min(22, slot - 3));
  const labelEvery = Math.max(1, Math.ceil(data.length / 6));
  return <ChartFrame title={title} detail={`${data.length} time bucket${data.length === 1 ? "" : "s"} · hover or focus`}>
    ...
    <svg class="timeline-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={data.map((datum) => `${datum.label}: ${datum.value}`).join(", ")}>
      {[0, .5, 1].map((fraction) => {
        const y = top + plotHeight * fraction;
        const value = Math.round(maximum * (1 - fraction));
        return <g key={fraction}><line class="grid-line" x1={left} y1={y} x2={width - 12} y2={y} /><text class="y-label" x={left - 8} y={y + 4} text-anchor="end">{value}</text></g>;
      })}
      {data.map((datum, index) => {
        const x = left + index * slot + (slot - barWidth) / 2;
        const barHeight = datum.value ? Math.max(2, (datum.value / maximum) * plotHeight) : 0;
        const y = top + plotHeight - barHeight;
        return <g class={`timeline-bucket${index === active ? " active" : ""}`} key={`${datum.label}-${index}`} onMouseEnter={() => setActive(index)} onFocus={() => setActive(index)} onClick={() => setActive(index)} tabIndex={0} aria-label={`${datum.label}: ${datum.value} tasks`}>
          <rect class="bar-hit" x={left + index * slot} y={top} width={slot} height={plotHeight} />
          <rect class="timeline-bar" x={x} y={y} width={barWidth} height={barHeight} rx="2" />
          {(index % labelEvery === 0 || index === data.length - 1) && <text class="x-label" x={x + barWidth / 2} y={top + plotHeight + 24} text-anchor="end" transform={`rotate(-35 ${x + barWidth / 2} ${top + plotHeight + 24})`}>{new Date(datum.label).toLocaleString(undefined, { month: "short", day: "numeric", hour: "numeric" })}</text>}
        </g>;
      })}
    </svg>
  </ChartFrame>;
...
export function StackedChart({ title, rows }: { title: string; rows: StackedRow[] }) {
  const [active, setActive] = useState({ row: 0, segment: 0 });
  const visible = rows.filter((row) => row.segments.some((segment) => segment.value > 0)).slice(0, 8);
  if (!visible.length) return null;
  ...
  return <ChartFrame title={title} detail="composition, not just rank">
    ...
    <div class="inline-legend"><span><i class="success" />Success</span><span><i class="error" />Error</span><span><i class="unknown" />Unknown</span></div>
  </ChartFrame>;
}

type RangeDatum = { label: string; min: number; median: number; p95: number; max: number };

export function RangePlot({ title, data }: { title: string; data: RangeDatum[] }) {
  const [active, setActive] = useState(0);
  ...
  const selected = visible[active] ?? visible[0];
  return <ChartFrame title={title} detail="minimum · median · P95 · maximum">
...
function SummaryChart({ section }: { section: Extract<ReportSection, { kind: "summary-visualization" }> }) {
  ...
  return <div class="summary-chart summary-viz-grid">
    {statusTotal ? <DonutChart title="Command Status Distribution" data={statusData} /> : <Empty>No result events to chart.</Empty>}
    {timeline.length ? <TimelineChart title="Command Volume Timeline" data={timeline} /> : <Empty>No task timestamps available for timeline.</Empty>}
...
function SectionBody({ section, query }: { section: ReportSection; query: string }) {
  switch (section.kind) {
    ...
    case "av-tracker": {
      ...
      return <><p class="section-summary">Scanned {section.scanned_task_count ?? 0} process-list task(s); found {section.detections?.length ?? 0} detection row(s).</p><DotPlot title="Detections by vendor" data={detections.map((row) => ({ label: `${row.vendor} · ${(row.matched_executables ?? []).join(", ")}`, value: row.occurrence_count }))} /><Table searching={Boolean(query)} label={section.title} headers={["Vendor", "Executables", "Occurrences", "Status", "Task"]} rows={detections.map((row, index) => ({ key: `${row.vendor}-${index}`, values: [row.vendor, (row.matched_executables ?? []).join(", "), row.occurrence_count, text(row.status), <Task task={row.task} />], sortValues: [row.vendor, (row.matched_executables ?? []).join(", "), row.occurrence_count, row.status, row.task.display_id ?? row.task.task_id] }))} /></>;
    }
    case "dwell-time": {
      const measurements = (section.measurements ?? []).filter((row) => matches(row, query));
      return <><dl class="inline-metrics"><dt>Median</dt><dd>{duration(section.median_seconds)}</dd><dt>P95</dt><dd>{duration(section.p95_seconds)}</dd><dt>Maximum</dt><dd>{duration(section.max_seconds)}</dd></dl><DotPlot title="Longest dwell intervals" formatValue={duration} data={measurements.map((row) => ({ label: `${row.from_task.command_name ?? row.from_task.task_id} #${row.from_task.display_id ?? row.from_task.task_id} → #${row.to_task.display_id ?? row.to_task.task_id}`, value: row.dwell_seconds, displayValue: duration(row.dwell_seconds) }))} /><Table searching={Boolean(query)} label={section.title} headers={["From", "To", "Dwell"]} rows={measurements.map((row, index) => ({ key: String(index), values: [<Task task={row.from_task} />, <Task task={row.to_task} />, duration(row.dwell_seconds)], sortValues: [row.from_task.display_id ?? row.from_task.task_id, row.to_task.display_id ?? row.to_task.task_id, row.dwell_seconds] }))} /></>;
    }
    case "parameter-entropy": {
      ...
      return <><DotPlot title="Highest parameter entropy" data={findings.map((row) => ({ label: `${row.task.command_name ?? "Task"} #${row.task.display_id ?? row.task.task_id} · ${row.finding_type}`, value: row.token_entropy ?? 0 }))} /><Table searching={Boolean(query)} label={section.title} headers={["Task", "Finding", "Entropy", "Detail"]} rows={findings.map((row, index) => ({ key: String(index), values: [<Task task={row.task} />, row.finding_type, text(row.token_entropy), row.detail], sortValues: [row.task.display_id ?? row.task.task_id, row.finding_type, row.token_entropy, row.detail] }))} /><Detail label={`${section.repeated_token_count ?? repeated.length} repeated high-entropy token(s)`}>{repeated.length ? <Table searching={Boolean(query)} label="Repeated high-entropy tokens" headers={["Prefix", "Mean entropy", "Occurrences", "Commands", "Detail"]} rows={repeated.map((row) => ({ key: row.token_prefix, values: [row.token_prefix, text(row.entropy_mean), row.occurrences, (row.commands ?? []).join(", "), row.detail], sortValues: [row.token_prefix, row.entropy_mean, row.occurrences, (row.commands ?? []).join(", "), row.detail] }))} /> : <Empty>No repeated high-entropy tokens.</Empty>}</Detail></>;
    }
    case "argument-position-profile": {
      const findings = (section.findings ?? []).filter((row) => matches(row, query));
      return <><p class="section-summary">Profiled {section.commands_profiled ?? 0} command(s), maximum argument depth {section.max_depth ?? 0}.</p><DotPlot title="Finding ratio by command and position" formatValue={(value) => percent(value)} data={findings.map((row) => ({ label: `${row.command_name} · position ${row.position ?? "—"} · ${row.finding_type}`, value: row.ratio ?? 0, displayValue: percent(row.ratio) }))} /><Table searching={Boolean(query)} label={section.title} headers={["Command", "Position", "Finding", "Occurrences", "Sample", "Ratio", "Detail"]} rows={findings.map((row, index) => ({ key: `${row.command_name}-${index}`, values: [row.command_name, text(row.position), row.finding_type, row.occurrences ?? 0, row.sample_size ?? 0, percent(row.ratio), row.detail], sortValues: [row.command_name, row.position, row.finding_type, row.occurrences ?? 0, row.sample_size ?? 0, row.ratio, row.detail] }))} /><Detail label="Depth distribution"><DotPlot title="Mean argument depth by command" data={(section.depth_distribution ?? []).map((row) => ({ label: `${row.command_name} · ${row.min_depth ?? 0}–${row.max_depth ?? 0} range`, value: row.mean_depth ?? 0 }))} /><Table searching={false} label="Argument depth distribution" headers={["Command", "Tasks", "Minimum", "Maximum", "Mean"]} rows={(section.depth_distribution ?? []).map((row) => ({ key: row.command_name, values: [row.command_name, row.task_count ?? 0, row.min_depth ?? 0, row.max_depth ?? 0, text(row.mean_depth)], sortValues: [row.command_name, row.task_count, row.min_depth, row.max_depth, row.mean_depth] }))} /></Detail><Detail label="Per-command profiles"><Table searching={false} label="Per-command argument profiles" headers={["Command", "Tasks", "Positions"]} rows={(section.command_profiles ?? []).map((row) => ({ key: row.command_name, values: [row.command_name, row.task_count ?? 0, row.positions ?? 0] }))} /></Detail></>;
    }
    case "tool-dump": {
      ...
      return <><DotPlot title="Tool matches by group" data={groups.map((row) => ({ label: `${row.name} · ${row.unique_command_count ?? 0} commands`, value: row.match_count }))} /><div class="card-grid">{groups.map((row) => <article class="finding-card" key={row.id} data-search-match={query ? "true" : undefined} tabIndex={query ? -1 : undefined}><h3>{row.name}</h3><strong>{row.match_count}</strong><p>{row.description}</p><small>{row.unique_command_count ?? 0} unique commands{row.artifact_path ? ` · ${row.artifact_path}` : ""}</small><Detail label="Matched tasks"><TaskList tasks={row.entries} empty="No task entries were retained for this group." /></Detail></article>)}</div></>;
    }
    case "data-quality": {
      const entries = (section.entries ?? []).filter((row) => matches(row, query));
      return <><StackedChart title="Result fidelity by source" rows={entries.map((entry) => ({ label: entry.source, segments: [{ label: "Success", value: entry.status_distribution?.success ?? 0, tone: "success" }, { label: "Error", value: entry.status_distribution?.error ?? 0, tone: "error" }, { label: "Unknown", value: (entry.status_distribution?.unknown ?? 0) + (entry.status_distribution?.other ?? 0), tone: "unknown" }] }))} /><div class="quality-grid">{entries.map((entry) => <div key={entry.source} data-search-match={query ? "true" : undefined} tabIndex={query ? -1 : undefined}><DataQuality entry={entry} /></div>)}</div></>;
    }
    case "run-diff": {
      const findings = (section.findings ?? []).filter((row) => matches(row, query));
      const summary = section.summary;
      return <><p class={`comparability ${section.comparability_status}`}>Comparability: {section.comparability_status}</p>{summary && <><dl class="inline-metrics"><dt>Regressions</dt><dd>{summary.likely_regressions ?? 0}</dd><dt>Improvements</dt><dd>{summary.likely_improvements ?? 0}</dd><dt>Low confidence</dt><dd>{summary.low_confidence_changes ?? 0}</dd><dt>Not comparable</dt><dd>{summary.not_comparable ?? 0}</dd></dl><DonutChart title="Comparison outcomes" data={[{ label: "Regressions", value: summary.likely_regressions ?? 0, tone: "error" }, { label: "Improvements", value: summary.likely_improvements ?? 0, tone: "success" }, { label: "Low confidence", value: summary.low_confidence_changes ?? 0, tone: "unknown" }, { label: "Not comparable", value: summary.not_comparable ?? 0, tone: "accent" }]} /></>}<DotPlot title="Metric change magnitude" data={findings.filter((row) => row.delta !== null && row.delta !== undefined).map((row) => ({ label: `${row.metric_id} · ${row.entity_id}`, value: row.delta ?? 0, displayValue: `${(row.delta ?? 0) > 0 ? "+" : ""}${text(row.delta)}` }))} /><Table searching={Boolean(query)} label={section.title} headers={["Metric", "Entity", "Classification", "Confidence", "Baseline", "Candidate", "Delta", "Explanation"]} rows={findings.map((row) => ({ key: `${row.metric_id}-${row.entity_id}`, values: [row.metric_id, row.entity_id, row.classification, row.confidence, text(row.baseline_value), text(row.candidate_value), text(row.delta), row.explanation], sortValues: [row.metric_id, row.entity_id, row.classification, row.confidence, row.baseline_value, row.candidate_value, row.delta, row.explanation] }))} /><Detail label="Entity presence changes"><Table searching={false} label="New and removed entities" headers={["Change", "Type", "Entity", "Count"]} rows={[...(section.new_entities ?? []).map((row) => ({ key: `new-${row.entity_type}-${row.entity_id}`, values: ["New", row.entity_type, row.entity_id, row.count ?? 0] })), ...(section.removed_entities ?? []).map((row) => ({ key: `removed-${row.entity_type}-${row.entity_id}`, values: ["Removed", row.entity_type, row.entity_id, row.count ?? 0] }))]} /></Detail></>;
    }
    case "outlier-context": {
      const outliers = (section.outliers ?? []).filter((row) => matches(row, query));
      return <><DotPlot title="Outlier duration by task" formatValue={duration} data={outliers.map((row) => ({ label: `${row.task.command_name ?? "Task"} #${row.task.display_id ?? row.task.task_id} · ${row.sequence_signature ?? "no signature"}`, value: row.duration_seconds, displayValue: duration(row.duration_seconds) }))} /><Table searching={Boolean(query)} label={section.title} headers={["Task", "Duration", "Signature"]} rows={outliers.map((row, index) => ({ key: String(index), values: [<Task task={row.task} />, duration(row.duration_seconds), text(row.sequence_signature)], sortValues: [row.task.display_id ?? row.task.task_id, row.duration_seconds, row.sequence_signature] }))} /><div class="detail-list">{outliers.map((row, index) => <Detail key={`${row.task.task_id}-${index}`} label={`Context for task ${row.task.display_id ?? row.task.task_id}`}><h3>Preceding tasks</h3><TaskList tasks={row.preceding} empty="No preceding task context." /><h3>Following tasks</h3><TaskList tasks={row.following} empty="No following task context." /></Detail>)}</div></>;
    }
